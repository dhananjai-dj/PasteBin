...
public class FolderService {
	...
	public ResponseEntity<?> createFolder(CreateFolderRequest createFolderRequest) {
		try {
			Folder folder = new Folder();
			folder.setFolderName(createFolderRequest.getFolderName());
			Long userId = createFolderRequest.getUserId();
			...
			return ResponseEntity.ok(GenericUtil.generateGenericResponse(true, "Folder has been generated!!!"));
		} catch (Exception e) {
	...
	public ResponseEntity<?> updateFolder(UpdateFolderRequest updateFolderRequest) {
		try {
			...
			return ResponseEntity.ok(GenericUtil.generateGenericResponse(true, "Folder has been updated!!!"));
		} catch (Exception e) {
	...
	public ResponseEntity<?> deleteFolder(Long folderId) {
		try {
			...
			return ResponseEntity.ok(GenericUtil.generateGenericResponse(true, "Folder has been deleted!!!"));
		} catch (Exception e) {
	...
	public ResponseEntity<?> getFolderById(Long folderId) {
		try {
			FolderDTO folderDTO = repositoryFacade.getFolderDTO(folderId);
			if (folderDTO == null) {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Unable to find the folder!!!");
			}
